...
uint64_t* allocate_symbolic_context() {
  return smalloc(7 * SIZEOFUINT64STAR + 10 * SIZEOFUINT64 + 4 * SIZEOFUINT64STAR + 1 * SIZEOFUINT64);
}
...
uint64_t* get_related_context(uint64_t* context) { return (uint64_t*) *(context + 21); }
...
void set_related_context(uint64_t* context, uint64_t* related) { *(context + 21) = (uint64_t) related; }
